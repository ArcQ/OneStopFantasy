...
def getDailyStats(year,month,day):
	...
	for i in range(0,24):
		...
		print i
		dailyStatsUrl = 'http://fantasysports.yahooapis.com/fantasy/v2/game/nba/players;start='+str(start)+';count='+str(count)+';status=ALL;sort=AR/stats'
		...
		for player in players.findall('player'):
			for child in player:
				if child.tag == 'player_id':
					...
					firstName = child.find('first').text
					lastName = child.find('last').text
					text_file.write(str(firstName) + '  ' + str(lastName) +'\n')



def run():
	getDailyStats('2015','03','09')

currentDate = (time.strftime("%Y-%m-%d"))
text_file = open("/home/ubuntu/Fydp/onestopfantasyassistant/BackEnd/OneStopFantasy/YqlConnect/stats/athleteRanks_"+currentDate+".txt", "a")
run()
text_file.close()
